...
class addSharing extends Component {
    ...
    changeType = (value, name) => {
        if (value === "CHARGE_ORDER_TYPE") {
            this.setState({[name]: {code: 'CHARGE_ORDER_TYPE'}});
        } else if (value === "INTERNET_PACKAGE_ORDER_TYPE") {
            this.setState({[name]: {code: 'INTERNET_PACKAGE_ORDER_TYPE'}});
        }
    };

    render() {
        const {multiplexedSaleInfoList,iban,amount,percent, merchants} = this.state;
        const option = [
                        ...
                        <div className="form-group mt-3 col-sm-6 col-md-3 float-right">
                            ...
                            >
                                {option.map(
                                    (info) => {
                                        return (
                                            <option selected={info.value === this.state.paymentTypes}
...
export default withRouter(addSharing);
